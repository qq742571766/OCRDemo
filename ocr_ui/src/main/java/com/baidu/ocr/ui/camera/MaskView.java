...
public class MaskView extends View {
    ...
    @IntDef({MASK_TYPE_NONE, MASK_TYPE_ID_CARD_FRONT, MASK_TYPE_ID_CARD_BACK,
            MASK_TYPE_BANK_CARD, MASK_TYPE_PASSPORT})
    @interface MaskType {
    }
    ...
    public void setMaskType(@MaskType int maskType) {
        ...
        switch (maskType) {
            case MASK_TYPE_ID_CARD_FRONT:
                locatorDrawable = ResourcesCompat.getDrawable(getResources(), R.drawable
                        .bd_ocr_id_card_locator_front, null);
                break;
            case MASK_TYPE_ID_CARD_BACK:
                locatorDrawable = ResourcesCompat.getDrawable(getResources(), R.drawable
                        .bd_ocr_id_card_locator_back, null);
                break;
            case MASK_TYPE_PASSPORT:
                locatorDrawable = ResourcesCompat.getDrawable(getResources(), R.drawable
                        .bd_ocr_passport_locator, null);
                break;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (maskType == MASK_TYPE_ID_CARD_FRONT) {
            locatorDrawable.setBounds((int) (left + 601f / 1006 * width), (int) (top + (110f /
                    632) * height), (int) (left + (963f / 1006) * width), (int) (top + (476f /
                    632) * height));
        } else if (maskType == MASK_TYPE_ID_CARD_BACK) {
            locatorDrawable.setBounds((int) (left + 51f / 1006 * width), (int) (top + (48f / 632)
                    * height), (int) (left + (250f / 1006) * width), (int) (top + (262f / 632) *
                    height));
        } else if (maskType == MASK_TYPE_PASSPORT) {
            locatorDrawable.setBounds((int) (left + 30f / 1006 * width), (int) (top + (20f / 632)
                    * height), (int) (left + (303f / 1006) * width), (int) (top + (416f / 632) *
                    height));
        }
    ...
    private void fillRectRound(float left, float top, float right, float bottom, float rx, float
            ry, boolean conformToOriginalPost) {
        path.reset();
        if (rx < 0) {
            rx = 0;
        }
